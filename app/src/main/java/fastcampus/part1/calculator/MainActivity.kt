...
class MainActivity : AppCompatActivity() {
    ...
    private val firstNumberText = StringBuilder("") // StringBuilder : 변경이 잦은 문자열 (↔️ String과 차이)
    ...
    fun operatorClicked(view: View) {
        ...
        if (operatorText.isNotEmpty()) {
            Toast.makeText(this, "연산자를 2번 이상 입력할 수 없습니다.", Toast.LENGTH_SHORT).show()
            return
        }
        if (secondNumberText.isNotEmpty()) {
            Toast.makeText(this, "1개의 연산자에 대한 계산이 가능합니다.", Toast.LENGTH_SHORT).show()
            return
        }
